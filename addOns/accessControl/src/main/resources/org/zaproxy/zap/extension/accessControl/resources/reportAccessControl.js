 ...
 ******************************************************************************/
	...
	/**
	 * Checks if a table cell has the given status class.
	 */
	function hasStatusClass(cell, statusClass) {
		if (cell.classList) {
			return cell.classList.contains(statusClass);
		}
		return (' ' + cell.className + ' ').indexOf(' ' + statusClass + ' ') > -1;
	}

	/**
	 * Returns true if the row contains any illegal access cell.
	 */
	function rowHasIllegalAccess(row) {
		for (var i = 0; i < row.cells.length; i++) {
			if (hasStatusClass(row.cells[i], "ILLEGAL")) {
				return true;
			}
		}
		return false;
	...
	function showAll() {
		...
		for (var r = 0; r < table.tBodies[0].rows.length; r++) {
			table.tBodies[0].rows[r].style.display = "";
		}
	}
	/**
	...
	function showIllegal() {
		...
		for (var i = 0; i < table.tBodies[0].rows.length; i++) {
			...
			row.style.display = rowHasIllegalAccess(row) ? "" : "none";
		}
	}
	/**
