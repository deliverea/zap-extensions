...
public class AccessControlJob extends AutomationJob {
                ...
                if (value instanceof ArrayList<?> rulesData) {
                    for (Object ruleData : rulesData) {
                        if (ruleData instanceof LinkedHashMap<?, ?> ruleMap) {
                            String nodeName = getEnv().replaceVars(ruleMap.get("uri"));;
                            @SuppressWarnings("unchecked")
    ...
    public void runJob(AutomationEnvironment env, AutomationProgress progress) {
        ...
        for (var userAccessRules : getData().getAccessRules().entrySet()) {
            ...
            for (var accessRule : userAccessRules.getValue()) {
                var node = targetNodes.stream().filter(sn -> sn.getHistoryReference().getURI().toString().equals(accessRule.getUri())
                        && accessRule.methods.contains(sn.getHistoryReference().getMethod())).findFirst();
                ...
                accessRulesManager.addRule(
                    userId,
                    new SiteTreeNode(node.get().getNodeName(), node.get().getHistoryReference().getURI()),
        ...
        File reportFile = new File(getParameters().getReportFile());
        ...
        try {
            ...
            progress.error(Constant.messages.getString("accessControl.automation.report.failed"));
        }
    }
    ...
    public static class Parameters extends AutomationData {
        ...
        private Boolean scanAsUnAuthUser = true;
    }
}
